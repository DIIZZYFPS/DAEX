...
- [ ] **Cold-start suggested prompts**: `generateSuggestedPrompts()` is genuinely contextual — it asks the model for starters based on the last 15 turns — but on a brand-new conversation there's no history to draw from, so the very first suggestions a user ever sees are generic. Seed the first-ever set from the model/hardware picked during onboarding and the "showcase" capability the user seemed most interested in (RAG vs. tools vs. voice), instead of an empty-history fallback prompt.
- [x] **Tool-aware system prompt**: available tools are passed to the engine structurally (`ConversationConfig.tools`), but the model is never told in prose what it can currently do. On models/configs where structural tool-calling is flaky, a short "you have access to: battery status, file search, ..." line in the system prompt would make tool use more reliable and let the model explain its own capabilities when asked. Shipped as a one-line capability summary (`DeviceTools.TOOL_CAPABILITY_SUMMARY`), appended only when tool calling is enabled. Verified on-device: asking "what tools do you have" now correctly lists all 8 device tools.
- [ ] **Cross-conversation search**: FTS5/BM25 hybrid search already exists for RAG documents, but there's no way to search across *past conversations* — only the currently-open thread is visible. Reuse the existing FTS infrastructure to index message content and add a search entry point from the Sidebar. (Note: `MessageEntity.embedding`/`DaexMemory.searchSimilarContext()` already exist for this but are dead code — embeddings are never populated at save time. Finishing that wiring is most of this item.)
- [ ] **Saved prompt library**: suggested prompts are one-shot and disappear once the chat starts. Let users pin a suggestion (or any past message) into a personal reusable prompt library, surfaced as an alternative to the "starter prompts" empty-state. (Note: `Message.isPinned`/`MessageEntity.isPinned` already exist and are persisted, just never set anywhere — reusable for this.)
- [ ] **Per-tool enable/disable**: "Native Tool Calling" is currently one global switch. As the tool registry grows (Phase 4), expose which individual tools are available per-conversation — useful for privacy (e.g. disable location/contacts tools by default) and for reducing prompt-injection surface from tool descriptions the model doesn't need.
