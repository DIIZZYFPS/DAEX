...
- [ ] **Tool-aware system prompt**: available tools are passed to the engine structurally (`ConversationConfig.tools`), but the model is never told in prose what it can currently do. On models/configs where structural tool-calling is flaky, a short "you have access to: battery status, file search, ..." line in the system prompt would make tool use more reliable and let the model explain its own capabilities when asked.
- [ ] **Cross-conversation search**: FTS5/BM25 hybrid search already exists for RAG documents, but there's no way to search across *past conversations* — only the currently-open thread is visible. Reuse the existing FTS infrastructure to index message content and add a search entry point from the Sidebar.
- [ ] **Saved prompt library**: suggested prompts are one-shot and disappear once the chat starts. Let users pin a suggestion (or any past message) into a personal reusable prompt library, surfaced as an alternative to the "starter prompts" empty-state.
...
- [ ] **Memory curation transparency**: global core memory is auto-curated in the background (`compactMemory`), but the user only sees a transient "[SYSTEM_LOG]: GLOBAL MEMORY CURATED" line. Surface a lightweight diff/summary ("learned: you prefer active-recall study techniques") so curation feels like a visible feature instead of an invisible background process the user has to trust blindly.
- [ ] **Per message read aloud button**: currently, voice read-aloud only works during live voice sessions. It would be nice to allow the user to read aloud any message in the chat by tapping a button on the message bubble.
