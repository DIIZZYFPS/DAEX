...
object FtsQuerySanitizer {
    private val STOP_WORDS = setOf(
        ...
        "wed", "well", "were", "werent", "weve", "what", "whats", "when", "whens", "where", "wheres", "which", "while",
    ...
    fun sanitize(query: String): String {
        ...
        val terms = clean.lowercase().split(Regex("\\s+")).filter { it.isNotBlank() }

        var filteredTerms = terms.filter { it !in STOP_WORDS }
        if (filteredTerms.isEmpty()) {
            filteredTerms = terms
        }

        return filteredTerms.joinToString(" OR ") { "\"$it\"*" }
    }
}
